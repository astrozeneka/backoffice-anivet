...
export default {
  ...
  methods: {
    ...
    async confirm(){
      ...
      let o = await beta_ajaxDelete(`/api/v1/data/admin/${idListSeparated.join(',')}`)
      if(o.affectedRows == idListSeparated.length){
        sessionstorage.setItem("message", `${this.count} admin${this.count>1?'s':''} has been deleted successfully`)
