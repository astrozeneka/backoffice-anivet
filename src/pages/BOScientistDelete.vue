    ...
    <div class="main">
      <div class="row">
        ...
        <div class="col-md-9">
          <header>
            <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="#">Scientist</a></li>
          ...
          <div>
            <h3>
              Are you sure to delete {{count}} row{{count>1?'s':''}} from the table Scientist ?
            </h3>
            <p>
...
export default {
  ...
  methods: {
    cancel(){
      window.location.href = `scientist-list.html`
    },
    async confirm(){
      let idListSeparated = (new URL(window.location.href)).searchParams.get("idList").split(",")
      let o = await beta_ajaxDelete(`/api/v1/data/scientist/${idListSeparated.join(',')}`)
      if(o.affectedRows == idListSeparated.length){
        sessionstorage.setItem("message", `${this.count} scientist${this.count>1?'s':''} has been deleted successfully`)
